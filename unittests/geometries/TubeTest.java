...
/*        Tube tube = new Tube(1d, new Ray(new Point3D(2,0,0), new Vector(0,0,1)));
        ...
        //TC06 ray is on the surface of the tube

        result = tube.findIntersections(new Ray(new Point3D(3,0,0), new Vector(0,0,1)));
        assertNull(result, "Wrong number of points");

        //TC07 ray is in center of the tube

        result = tube.findIntersections(new Ray(new Point3D(2,0,0), new Vector(0,0,1)));
        assertNull(result, "Wrong number of points");

        //TC08 ray is perpendicular to tube's ray and starts from outside the tube

        result = tube.findIntersections(new Ray(new Point3D(-2,0,0), new Vector(1,0,0)));
        assertEquals(2, result.size(), "Wrong number of points");
        assertEquals(List.of(new Point3D(1,0,0), new Point3D(3,0,0)), result, "Bad intersection points");

        //TC09 ray is perpendicular to tube's ray and starts from inside tube (not center)

        result = tube.findIntersections(new Ray(new Point3D(1.5,0,0), new Vector(1,0,0)));
        assertEquals(1, result.size(), "Wrong number of points");
        assertEquals(List.of(new Point3D(3,0,0)), result, "Bad intersection points");

        //TC10 ray is perpendicular to tube's ray and starts from the center of tube

        result = tube.findIntersections(new Ray(new Point3D(2,0,0), new Vector(1,0,0)));
        assertEquals(1, result.size(), "Wrong number of points");
        assertEquals(List.of(new Point3D(3,0,0)), result, "Bad intersection points");

        //TC11 ray is perpendicular to tube's ray and starts from the surface of tube to inside

        result = tube.findIntersections(new Ray(new Point3D(1,0,0), new Vector(1,0,0)));
        assertEquals(1, result.size(), "Wrong number of points");
        assertEquals(List.of(new Point3D(3,0,0)), result, "Bad intersection points");

        //TC12 ray is perpendicular to tube's ray and starts from the surface of tube to outside
        ...
        result = tube.findIntersections(new Ray(new Point3D(3,0,0), new Vector(1,1,1)));
        assertNull(result, "Wrong number of points");

        //TC14 ray starts from the surface to inside

        result = tube.findIntersections(new Ray(new Point3D(3,0,0), new Vector(-1,0,1)));
        assertEquals(1, result.size(), "Wrong number of points");
        assertEquals(List.of(new Point3D(1,0,2)), result, "Bad intersection point");

        //TC15 ray starts from the center

        result = tube.findIntersections(new Ray(new Point3D(2,0,0), new Vector(1,0,1)));
        assertEquals(1, result.size(), "Wrong number of points");
        assertEquals(List.of(new Point3D(3,0,1)), result, "Bad intersection point");

        //TC16 prolongement of ray crosses tube
